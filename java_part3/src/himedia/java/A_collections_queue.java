...
// - remove() : 큐의 맨 앞에 있는 요소를 제거하고 반환한다. 큐가 비어있는 경우 예외를 발생시킨다.
// - poll() : 큐의 맨 앞에 있는 요소를 제거하고 반환한다. 큐가 비어 있으면 null을 반환한다.
// - element() : 큐의 맨 앞에 있는 요소를 반환하지만, 제거하지는 않는다. 큐가 비어있는 경우 예외를 발생시킨다.
// - peek() : 큐의 맨 앞에 있는 요소를 반환하지만, 제거하지는 않는다. 큐가 비어있는 경우 예외를 발생시킨다.
...
class Graph {
    ...
    //간선추가
    public void addEdge(int v, int w){//인접리스트 w
        graphList[v].add(w);
        graphList[w].add(v);
    }
...
public class A_collections_queue {
    ...
    // 1. LinkedList
    // - LinkedList는 Queue인터페이스를 구현하므로, 큐를 사용할 수 있다.
    ...
    // PriorityQueue
    // - 요소가 자연순서(기본순서)나 제공된 비교기(Comparator)에 따라 결정된다.
    ...
    public static void exam2(){
        ...
        priorityQueue.forEach(
                element-> System.out.println("Elment : "+ element)
        );
    }
...
//        }
        ...
        // 반드시 poll()이나 remove()와 같은 메서드를 사용해 요소를 꺼내면서 확인해야 한다.
...
//        Boolean visited1 = false; // 값을 안 넣으면 null;로 설정 이미 초기화 되어있음
