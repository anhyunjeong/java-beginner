...
// 반면에 start()는 새로운 스레드가 작업을 실행하는데 필요한 호출스택을 생성한 다음에 run()을 호출해서, 생성된 호출스택에 run()이 첫 번째로 올라가게 된다.
// 모든 스레드는 독립적인 작업을 수행하기 위해 자신만의 호출스택을 필요로 하기 때문에 새로운 스레드를 생성하고 실행시킬 때마다 새로운 호출스택이 생성괴고
// 스레드가 종료되면 작업에 사용된 호출스택은 소멸된다.

// 스레드가 둘 이상일 때는 호출스택의 최상위에 있는 메서드 일지라도 대기상태 일 수 있다.
// 각 스레드들은 작성된 스케줄러에 따라 자신의 순서가 되면 지정된 시간동안 작업을 수행한다.
// 이 때 주어진 시간동안 작업을 마치지 못한 스레드는 다시 자신의 차례가 돌아올 때까지 대기상태로 있게 된다.
...
public class C_thread_2 {
    ...
    public static void exam2(){
        C_threadEx_exception t1=new C_threadEx_exception();
        t1.run();//스레드 생성x
    }
